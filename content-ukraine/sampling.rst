...
Ми записуємо значення :math:`S(t)` через рівні проміжки часу тривалістю по :math:`T` секунд, відомі як **період вибірки**. Частота, з якою ми беремо вибірку, тобто кількість вибірок, взятих за секунду, є просто :math:`\frac{1}{T}`. Ця величина називається **частотою дискретизації**, і вона зворотна до періоду дискретизації. Наприклад, якщо ми маємо частоту дискретизації 10 Гц, то період дискретизації становить 0,1 секунди; між кожним двома виборками буде 0,1 секунда. На практиці наші частоти дискретизації будуть в діапазоні від сотень кГц до десятків МГц або навіть більше. Коли ми оцифровуємо сигнали, ми повинні пам’ятати про частоту дискретизації, це дуже важливий параметр.
...
Дискретизацію IQ легше зрозуміти, використовуючи точку зору передавача.  Розгляньмо задачу передачі радіосигналу через повітря на певній частоті :math:`f` (у Гц).  Ми хочемо взяти цю синусоїду частоти :math:`f` і керувати її амплітудою :math:`A` та фазою :math:`\phi`:

.. math::

 A \cos(2 \pi f t - \phi)

Знак мінус - це суто питання домовленості, і він не важливий для розуміння концепції. У будь-який момент часу ми можемо хотіти передати іншу амплітуду і фазу, тому, щоб бути більш формальними, ми можемо представити їх як функції часу:

.. math::

 A(t) \cos(2 \pi f t - \phi(t))

Виявляється, що в радіочастотних схемах керувати амплітудою синусоїди легко, а керувати фазою важко, тож ми можемо скористатися тригонометричною тотожністю: :math:`a \cos(x) + b \sin(x) = A \cos(x - \phi)`, яка говорить нам, що сума cos() і sin() однакової частоти, кожен з фазою 0, еквівалентна одному cos() з амплітудою :math:`A` і фазою :math:`\phi`.  Використовуючи I і Q замість :math:`a` і :math:`b` та додавши назад наше :math:`2 \pi f t`, ми отримуємо:
...
Використовуючи цей підхід з I і Q, ми можемо передати будь-які величину і фазу, які захочемо, за допомогою схеми, що виглядає приблизно так:
...
Припустимо, у нас є відлік IQ, тобто одне комплексне число :math:`I + jQ`.  Ми можемо **промодулювати** цим відліком IQ синусоїду; амплітуда і фаза визначаються цим відліком IQ:
...
Хоча ми вже побачили математику, погляньмо на додавання двох синусоїд, зсунутих на 90 градусів у фазі.  В інтерактивному застосунку нижче є повзунок для регулювання I і ще один для регулювання Q, тобто амплітуд косинуса і синуса.  На графіку зображені косинус (червоний), синус (синій) і їхня сума (зелений).
