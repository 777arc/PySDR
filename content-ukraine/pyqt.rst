...
- **Багатопоточність**: Qt має вбудовану підтримку багатопоточності та надає класи для створення робочих потоків, що працюють у фоновому режимі.  Багатопоточність дозволяє виконувати тривалі операції, не блокуючи головний GUI-потік.
...
Запустіть цей код: у консолі що секунду з'являтиметься «Starting run()», і кнопка все ще працюватиме без затримок.  Поки що в робочому потоці ми лише друкуємо та «спимо», але скоро додамо туди керування SDR та DSP-код.
...
.. raw:: html
   ...
   <details>
   <summary>Розгорнути код для порівняння</summary>
...
Дуже важливо пам'ятати, що :code:`QSlider` працює з цілими числами, тож, задаючи діапазон 0…73, ми дозволяємо повзунку обирати лише цілі значення (включно з початком і кінцем).  :code:`setTickInterval(2)` — це суто візуальний ефект.  Саме тому ми використовуємо кілогерци як одиниці для повзунка частоти, щоб отримати крок у 1 кГц.
...
У цьому випадку у нас є функція з одним рядком коду, і ми посилаємось на неї лише один раз — коли задаємо зворотний виклик :code:`connect`.  У таких ситуаціях можна використати лямбда-функцію — спосіб визначити функцію в одному рядку.  Ось попередній код, переписаний з лямбда-функцією:
...
І ось як це врешті виглядає:

.. image:: ../_images/pyqt_time_plot.png
   :scale: 50 % 
   :align: center
   :alt: Графік у часовій області PyQtGraph

Схожий підхід ми застосуємо і для графіка частотної області (PSD).

**********************
ImageItem із PyQtGraph
**********************

Аналізатор спектра неповний без «водоспаду» (тобто спектрограми реального часу), і для нього ми використаємо ImageItem із PyQtGraph, який відмальовує зображення з 1, 3 або 4 «каналами».  Один канал означає, що ви передаєте йому 2D-масив чисел з рухомою комою або цілих, а він застосовує таблицю відповідності (LUT), щоб накласти колірну карту і врешті створити зображення.  Як альтернативу можна передати RGB (3 канали) або RGBA (4 канали).  Ми обчислюватимемо нашу спектрограму як 2D-масив NumPy з дійсними числами і передаватимемо його безпосередньо в ImageItem.  Ми оберемо колірну карту й навіть скористаємося вбудованою можливістю показувати графічну LUT, яка відображає розподіл значень наших даних і те, як накладається колірна карта.

Власне ініціалізація «водоспаду» доволі проста: ми використовуємо PlotWidget як контейнер (щоб і далі мати підписані осі x та y), а потім додаємо до нього ImageItem:
...
Слот (функція зворотного виклику), який оновлює дані «водоспаду» і живе в :code:`__init__` :code:`MainWindow`, виглядає так:
...
де spectrogram — це 2D-масив NumPy з дійсними числами.  Крім установлення даних зображення, ми обчислюємо мінімум і максимум для колірної карти на основі середнього значення та стандартного відхилення даних, які знадобляться нам пізніше.  Остання частина GUI-коду для спектрограми — створення колірної шкали, яка водночас задає використану колірну карту:
...
Другий рядок є важливим: саме він з'єднує цю колірну шкалу з ImageItem.  У цьому ж коді ми обираємо колірну карту та задаємо початкові рівні (у нашому випадку від -30 дБ до +20 дБ).  У коді робочого потоку ви побачите, як обчислюється і зберігається 2D-масив спектрограми.  Нижче наведено знімок цієї частини GUI, який демонструє неймовірні вбудовані можливості колірної шкали та відображення LUT; зверніть увагу, що повернута набік дзвоноподібна крива — це розподіл значень спектрограми, і бачити його дуже корисно.

.. image:: ../_images/pyqt_spectrogram.png
   :scale: 50 % 
   :align: center
   :alt: Спектрограма та колірна шкала PyQtGraph

***********************
Робочий потік
***********************

Пригадайте, що на початку розділу ми навчилися створювати окремий потік за допомогою класу, який назвали SDRWorker, із функцією run().  Саме туди ми покладемо весь наш SDR- та DSP-код, за винятком ініціалізації SDR, яку поки що виконуємо глобально.  Робочий потік також відповідатиме за оновлення трьох графіків: він випромінюватиме сигнали, коли з'являються нові вибірки, щоб запустити функції зворотного виклику, які ми вже створили в :code:`MainWindow` і які врешті оновлюють графіки.  Клас SDRWorker можна розділити на чотири частини:

#. :code:`init()` — використовується для ініціалізації стану, наприклад 2D-масиву спектрограми
#. Сигнали PyQt — ми маємо визначити власні сигнали, які випромінюватимемо
#. Слоти PyQt — функції зворотного виклику, які запускаються подіями GUI, як-от переміщенням повзунка
#. :code:`run()` — головний цикл, що виконується безперервно

***********************
Сигнали PyQt
***********************

У GUI-коді нам не доводилося визначати сигнали, бо вони вже вбудовані у віджети, якими ми користувалися, як-от :code:`valueChanged` у :code:`QSlider`.  Наш клас SDRWorker — власний, тож усі сигнали, які ми хочемо випромінювати, треба визначити ще до того, як ми почнемо викликати run().  Ось код класу SDRWorker, який визначає чотири сигнали, що ми використовуватимемо, та відповідні їм типи даних:
...
Перші три сигнали передають один об'єкт — масив NumPy.  Останній сигнал не несе з собою жодного об'єкта.  Можна також передавати кілька об'єктів одночасно, просто перелічивши типи даних через кому, але для нашого застосунку це не потрібно.  Будь-де всередині run() ми можемо випромінити сигнал у GUI-потік лише одним рядком коду, наприклад:

.. code-block:: python

    self.time_plot_update.emit(samples)

Залишився ще один крок, щоб усі з'єднання сигналів і слотів запрацювали: у GUI-коді (у самому кінці :code:`__init__` класу :code:`MainWindow`) ми маємо з'єднати сигнали робочого потоку зі слотами GUI, наприклад:

.. code-block:: python

    worker.time_plot_update.connect(time_plot_callback) # connect the signal to the callback

Пам'ятайте, що :code:`worker` — це екземпляр класу SDRWorker, який ми створили в GUI-коді.  Отже, вище ми з'єднуємо сигнал робочого потоку :code:`time_plot_update` зі слотом GUI :code:`time_plot_callback`, який визначили раніше.  Зараз саме час повернутися й переглянути показані досі фрагменти коду та побачити, як вони поєднуються, щоб переконатися, що ви розумієте, як GUI і робочий потік взаємодіють, адже це ключова частина програмування на PyQt.

***********************
Слоти робочого потоку
***********************

Слоти робочого потоку — це функції зворотного виклику, які запускаються подіями GUI, як-от переміщенням повзунка підсилення.  Вони доволі прості; наприклад, цей слот оновлює значення підсилення SDR на нове, обране повзунком:
...
*****************************
Функція run() робочого потоку
*****************************

Функція :code:`run()` — це місце, де відбувається вся цікава DSP-частина!  У нашому застосунку кожен виклик run починатиметься з отримання набору вибірок від SDR (або з моделювання вибірок, якщо SDR у вас немає).  
