...
If we do not have any imaginary component in our signal, then we don't have any Q values (or you can think of all Q values equal to zero). This, in turn, means that we only have cosine signals without any phase shift. A sum of cosine signals will be symmetrical around the y-axis when plotting frequency domain, due to having the same positive and negative components.
