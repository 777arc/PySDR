...
З огляду на назву PySDR ви можете подумати, що Python є критично важливою частиною цього ресурсу, але насправді вибір мови програмування тут не такий вже й принциповий.  В епоху ШІ переписати код з однієї мови на іншу - тривіальна задача.  У цьому підручнику **ми використовуємо Python майже як різновид псевдокоду**, з тим бонусом, що його справді можна запустити, побачити результати, побудувати графіки сигналів, перебрати параметри тощо.  Python обрано просто тому, що він безкоштовний, легко запускається на всіх платформах, потребує мало шаблонного коду, має легкий синтаксис, добре читається і має величезну екосистему бібліотек та прикладів коду.  Допомагає й те, що більшість SDR мають Python API.

PySDR навмисно не містить власної бібліотеки Python чи будь-яких функцій-обгорток - увесь код написано чистим Python із використанням стандартних бібліотек, таких як NumPy (стандартна бібліотека для масивів і математики високого рівня), SciPy (більш специфічні для ЦОС функції, наприклад проєктування фільтрів) та Matplotlib (побудова графіків, що дозволяє візуалізувати сигнали).

Зауважте, що хоча Python загалом "повільніший" за C/C++, більшість функцій у Python/NumPy насправді реалізовано на C/C++ під капотом і добре оптимізовано, тож ви можете здивуватися, наскільки швидко ЦОС на процесорі може працювати в Python.  Так само SDR API, які ми використовуємо (наприклад, UHD), - це просто набір Python-прив'язок до функцій/класів на C/C++.  У реальних розгорнутих радіосистемах високошвидкісна обробка сигналів усе одно зазвичай реалізується у ПЛІС!

Деякі глави PySDR містять приклади коду, які можна відкрити як вебовий Jupyter-ноутбук (за допомогою JupyterLite), що дозволяє експериментувати з прикладами на Python і запускати їх прямо з браузера, нічого не встановлюючи.  Щоб перевірити, чи це працює у вашому браузері, спробуйте відкрити `цей приклад <../jupyterlite/notebooks/index.html?path=example.ipynb>`_.
