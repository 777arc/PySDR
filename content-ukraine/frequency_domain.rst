...
.. image:: ../_images/square-wave.svg
   ...
   :alt: Часово-частотна пара Фур'є прямокутної хвилі, яка є sinc-функцією (sin(x)/x) у частотній області
...
3. Масштабування у часовій області:

.. math::
   x(at) \leftrightarrow X\left(\frac{f}{a}\right)

У лівій частині рівняння ми бачимо, що ми масштабуємо наш сигнал x(t) у часовій області. Нижче приклад масштабованого і немасштабованого сигналу в часі, а зліва показано те, що відбувається з їх частотними версіями.
...
Тепер повернемося до перетворення Фур'є. Я показав вам рівняння для дискретного перетворення Фур'є, але 99.9% часу ви будете використовувати під час кодування функцію ШПФ, fft().  Швидке перетворення Фур'є (ШПФ) - це просто алгоритм для обчислення дискретного перетворення Фур'є.  Його було розроблено десятки років тому, і хоча існують різні варіанти реалізації, він все ще залишається лідером з швидкості обчислення дискретного перетворення Фур'є. Що доречно, зважаючи на слово "швидке" в його назві.

ШПФ - це функція з одним входом і одним виходом.  Вона перетворює сигнал з часової в частотну область: 
...
У цьому підручнику ми розглядатимемо лише одновимірні ШПФ (двовимірні використовуються для обробки зображень та інших застосувань). Для наших цілей вважатимемо, що функція ШПФ має один вхід: вектор відліків, і один вихід: частотну версію цього вектора відліків.  Розмір виходу завжди дорівнює розміру входу. Якщо ми подамо на вхід ШПФ 1,024 відліки, то отримаємо 1,024 на виході.  Збиває з пантелику, що результат що ми отримуємо в частотній області, тобто "ширина" його графіку по осі х не змінюється з зміною кількості відліків на вході в часовій області.  Давайте візуалізуємо це, нижче приведені вхідні та вихідні масиви разом з індексами та одиницями виміру для відліків:
...
Оскільки вихідні дані знаходяться в частотній області, ширина діапазону по осі х залежить від частоти дискретизації, частота дискритезації буде розглянута в наступній главі.  Якщо ми візьмемо більше відліків для вхідного вектора, ми отримуємо кращу роздільну здатність у частотній області (до того ж за один раз обробляємо більше відліків).  Насправді ми не "бачимо" більш ширший частотний діапазон, маючи довший вхідний сигнал. Єдиний спосіб збільшити ширший частотний діапазон це збільшити частоту дискретизації (зменшити період дискретизації :math:`\Delta t`).
...
Щодо частотного інтервалу, то кожен відлік іде через проміжок рівний :math:`f_s/N` Гц, тобто подача більшої кількості відліків на вхід ШПФ призведе до вищої роздільної здатності на виході, але в тому ж частотному діапазоні по ширині.  Дуже незначна деталь, яку можна проігнорувати, якщо ви новачок: математично останній індекс не відповідає *точно* :math:`f_s/2`, а відповідає :math:`f_s/2 - f_s/N`, що для великих значень :math:`N` приблизно дорівнює :math:`f_s/2`.

********************
Від'ємні частоти
********************

Що таке від'ємна частота?  Наразі просто вважайте, що це пов'язано з використанням комплексних чисел (уявних чисел) - насправді не існує такого поняття, як "від'ємна частота", коли мова йде про передачу/прийом радіосигналів, це просто представлення, яке ми використовуємо.  Можно думати про це наступним чином.  Уявімо, що ми налаштували SDR  на частоту 100 МГц (FM-діапазон) і робимо оцифровку з частотою дискритизації 10 МГц.  Іншими словами, ми будемо бачити спектр від 95 МГц до 105 МГц.  Нехай в цьому діапазоні присутні три сигнали:
...
Порада: незалежно від того, що ви робите, якщо ви зіткнулись з комплексними числами, спробуйте обчислити амплітуду і фазу і подивіться, може це додасть більше розуміння.  Давайте так і зробимо, і побудуємо графік амплітуди і фази.  У більшості мов для знаходження амплітуди комплексного числа є функція abs().  Функція для фази може бути різною в різних мовах, але у Python ми можемо скористатися :code:`np.angle()` з Numpy, яка повертає фазу в радіанах.
...
Наразі ми не додаємо розмірності для вісі x для графіків, відображаються лише індекси відліків в масиві виходу (індекс починається з 0).  В алгоритмі ШПФ з математичних міркувань, вихідні дані після обчислення мають наступний формат:
...
Але нам би зручніше було мати 0 Гц (постійний струм) в центрі, а від'ємні частоти зліва (так як зазвичай ми представляємо графік в частотній області).  Отже, кожного разу, коли ми робимо ШПФ, нам потрібно виконати "зсув ШПФ", який є простою операцією перегрупування масиву, на кшталт кільцевого зсуву, але більше схожого на "покладіть це сюди, а це туди".  На наведеній нижче схемі повністю описано, що робить операція зсуву ШПФ:
...
Зверніть увагу, що ми бачимо наш пік на частоті 0.15 Гц, тобто на частоті, яку ми використовували при створенні синусоїди. Це означає, що наше ШПФ спрацювало!  Якби ми не знали коду, який використовувався для генерації синусоїди, а нам просто дали список зразків, ми могли б використати ШПФ для визначення частоти. Причина, чому ми бачимо пік на частоті -0,15 Гц, пов'язана з тим, що це був дійсний , а не комплексний сигнал, цей випадок ми глибше розглянемо пізніше.

******************************
Віконна функція
******************************

Коли ми використовуємо ШПФ для вимірювання частотних складових нашого сигналу, ми припускаємо, що ШПФ обробляє тільки фрагмент *періодичного* сигналу.  А результат ШПФ над фрагментом такий, ніби поданий фрагмент сигналу продовжується далі повторючись до нескінченності зправа і зліва. Тобто останній відлік фрагменту має з'єднуватися з першим відліком наступного фрагменту, але так як фрагмент циклічно повторюються - то це ж і є першим відліком нашого фрагменту.  Це випливає з теорії, що лежить в основі перетворення Фур'є.  Для нас це означає, що насправді для довільного сигналу наш останній і перший відлік можуть не плавно з'єднуватись, через що виникають різкі перепади амплітуди між цими кінцевими відліками, а ці різкі перепади в часовій області приведуть в частотній області до виникнення багатьох додаткових частот.  Простіше кажучи, якщо ми робимо ШПФ зі 100 відліків, використовуючи :code:`np.fft.fft(x)`, ми хочемо, щоб :code:`x[0]` і :code:`x[99]` були рівними або близькими за значенням.

Для того щоб зкомпенсувати цей перепад при циклічному повторенні фрагменту ми використовуємо "віконну функцію".  Безпосередньо перед операцією ШПФ ми множимо фрагмент сигналу на віконну функцію, яка дорівнює довжині фрагменту і може бути будь-якою функцією, що спадає до нуля на обох кінцях.  Це гарантує, що фрагмент сигналу буде починатися і закінчуватися в нулі і таким чином отримані фрагменти плавно з'єднуються.  До поширених віконних функцій належать функції Геммінга, Ганнінга, Блекмана та Кайзера.  Якщо не застосовується жодна віконна функція, це називається використанням "прямокутного" вікна, тому що це еквіввалентно множення фрагменту на масив одиниць.   Ось як виглядають деякі віконні функції:

.. image:: ../_images/windows.svg
   :align: center
   :target: ../_images/windows.svg
   :alt: Віконні функції у часовій та частотній області для прямокутних, гаммінгових, ханнінгових, бартлетових, блекманівських та кайзерівських вікон

Простим підходом для початківців є використання вікна Гемінга, яке можна створити у Python за допомогою :code:`np.hamming(N)`, де N - це кількість елементів у масиві, що є розміром вашого фрагменту для ШПФ.  У наведеному вище прикладі ми застосуємо вікно безпосередньо перед ШПФ. Після 2-го рядка в коді треба вставити

.. code-block:: python

 s = s * np.hamming(100)

Якщо ви боїтеся обрати неправильне вікно, не бійтеся.  При викорстані вікна різниця між Hamming, Hanning, Blackman і Kaiser мінімальна у порівнянні з тим ефектом, який отримаємо якщо взагалі їх не використати, оскільки всі вони плавно знижують значання амплітуд фрагментів до нуля з обох боків і вирішують основну проблему з'єднання.

Якщо хочете побачити спектральне просочування на власні очі, схема нижче - це справжній GNU Radio, скомпільований для роботи просто у вашому браузері за допомогою `GNU Radio World <https://gnuradioworld.com/examples/analog/frequency-domain-windowing/>`_.  Вона генерує два тони: один сильний, а другий - приблизно на 50 дБ слабший, розташований поруч, - і будує спектр.  Запустіть її, і виявиться, що слабкого тону ніде не видно: він похований під просочуванням частот від потужного сусіда, бо не було застосовано жодного вікна (або тобто застосовано прямокутне вікно).  Не зупиняючи запущену схему, клацніть середньою кнопкою миші на графіку частотної області й перемкніть "FFT Window" на Blackman - і слабкий тон одразу проявиться.  Спробуйте інші віконні функції, щоб побачити, що станеться.  Заразом спробуйте змінити й розмір ШПФ.
...
де :math:`w^k_N = e^{j2\pi k/N}` це комплексний коефіцієнт обертання (:math:`N` - розмір під-шПФ, а :math:`k` - індекс).  Зауважте, що вхідні та вихідні дані мають бути комплексними, наприклад, :math:`x_0` може бути 0.6123 - 0.5213j, і суми/множники є також комплексними.
...
Кожен стовпчик у цьому шаблоні є набором операцій, які можна виконувати паралельно, за :math:`log_2(N)` кроків, тому обчислювальна складність ШПФ становить O(:math:`N\log N`), тоді як ДПФ - O(:math:`N^2`).
...
**************************
Прямокутний імпульс і sinc
**************************

Уявіть, що ви вмикаєте сигнал, утримуєте його на сталому рівні рівно одну секунду, а потім вимикаєте.  У часовій області це чи не найпростіший імпульс, який можна створити: прямокутник із пласкою вершиною завширшки одна секунда.  Ця форма настільки часто трапляється в ЦОС, що має власну назву - прямокутний імпульс (англійською boxcar), бо він схожий на товарний вагон, що стоїть на осі часу.  Природно запитати: якщо в часі імпульс такий простий, як він виглядає в частотній області?

Можна було б припустити, що простий імпульс має простий спектр, але виявляється, що все навпаки.  Візьміть ШПФ від прямокутного імпульсу - і ви отримаєте форму, показану праворуч нижче: високий центральний горб, оточений брижами, які згасають у міру віддалення від центру.  Ця форма називається sinc (вимовляється "сінс").  На графіку нижче показано цю пару поруч: прямокутний імпульс у часовій області ліворуч і його sinc-спектр праворуч.

.. image:: ../_images/boxcar_sinc.svg
   :align: center
   :target: ../_images/boxcar_sinc.svg
   :alt: Прямокутний імпульс у часовій області та його спектр у формі sinc

Зверніть увагу, що спектр не закінчується на першому провалі - він продовжується дедалі меншими горбами.  Ці горби називаються бічними пелюстками, а високий горб посередині - головною пелюсткою.  Причина, з якої звичайний прямокутник породжує всю цю структуру, - різкі краї: миттєве вмикання й вимикання сигналу потребує енергії, розподіленої в широкому діапазоні частот, і sinc - це саме те, як ця енергія розподіляється.

Математика підтверджує цю картину.  Якщо наш прямокутний імпульс має амплітуду :math:`A` і ширину :math:`T` (у секундах), його перетворення Фур'є має вигляд:

.. math::
   X(f) = A T \, \mathrm{sinc}(f T)

де сама функція sinc визначається як:

.. math::
   \mathrm{sinc}(x) = \frac{\sin(\pi x)}{\pi x}

Спектр прямокутника - це sinc, масштабований за висотою на :math:`AT` і розтягнутий за частотою на :math:`T`. Головна пелюстка вперше перетинає нуль, коли :math:`fT = 1`, тобто на частоті :math:`f = 1/T`, отже спектр стає тим вужчим, чим ширшим є імпульс. Так само, як ми бачили з властивістю масштабування, короткий у часі імпульс є широким за частотою, а широкий у часі імпульс - вузьким за частотою.  Ці дві величини пов'язані обернено: неможливо зробити щось компактним одночасно в обох областях.  Якщо вам потрібен сигнал, який займає дуже вузьку смугу, доведеться дозволити йому тривати довго, а якщо потрібен дуже короткий імпульс, доведеться змиритися з тим, що він розмажеться по багатьох частотах.

Анімація нижче добре це ілюструє.  Подивіться, що відбувається, коли прямокутний імпульс ліворуч, спочатку дуже короткий, поступово розширюється.  Коли імпульс "товстішає" в часі, його sinc-спектр праворуч стискається дедалі тісніше до центру - точно так, як передбачає :math:`f = 1/T`.

.. image:: ../_images/boxcar_sinc_animation.gif
   :align: center
   :target: ../_images/boxcar_sinc_animation.gif
   :alt: Анімація розширення прямокутного імпульсу в часі, тоді як його sinc-спектр звужується за частотою
